...
export function BrigadeCard({ brigade }: { brigade: BrigadeWithAll }) {
  const [addingMember, setAddingMember] = useState(false);
  const [removePending, startRemoveTransition] = useTransition();
  const [deletePending, startDeleteTransition] = useTransition();

  // Filter only active SMRs (where active === true)
  const activeSmrs = brigade.smr.filter((s) => s.active);

  // Get unique PodObekti count where this brigade is active
  const activePodObektiCount = new Set(activeSmrs.map((s) => s.podObektId))
    .size;
  ...
  return (
    <div className="border border-line rounded-xl bg-surface overflow-hidden">
      {/* Card Header */}
      <div className="flex items-start justify-between gap-3 px-4 pt-4 pb-3 border-b border-line/50">
        <div>
          <div className="flex items-center gap-2">
            <p className="text-sm font-semibold text-ink">{brigade.name}</p>
            {activeSmrs.length > 0 && (
              <span className="flex items-center gap-1 text-[10px] font-medium bg-green-500/10 text-green-600 dark:text-green-400 px-2 py-0.5 rounded-full">
                <span className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse" />
                Активна ({activeSmrs.length})
              </span>
            )}
          </div>
          <div className="flex items-center gap-2 text-xs text-ink-4 mt-0.5">
            <span>{brigade.members.length} члена</span>
            {activePodObektiCount > 0 && (
              <>
                <span className="text-ink-ghost">•</span>
                <span>{activePodObektiCount} обекта</span>
              </>
            )}
          </div>
        </div>
        ...
        <button
          ...
          className="text-ink-4 hover:text-red-400 transition-colors shrink-0 disabled:opacity-50 p-1 rounded-lg hover:bg-lift"
        >
          <svg
            className="w-4 h-4"
            fill="none"
      ...
      <div className="px-4 py-3 space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          {brigade.members.length === 0 && !addingMember ? (
            <p className="text-xs text-ink-5 py-1">Няма добавени членове.</p>
          ) : (
            brigade.members.map((m) => (
              <div
                key={m.id}
                className="flex items-center justify-between gap-2 py-1.5 px-2.5 bg-well rounded-lg border border-line/40"
              >
                <div className="min-w-0 flex items-center gap-2">
                  <span className="text-xs text-ink font-medium">{m.name}</span>
                  {m.role && (
                    <span className="text-[10px] text-ink-4 bg-lift px-1.5 py-0.5 rounded border border-line/50">
                      {m.role}
                    </span>
                  )}
                </div>
                <button
                  ...
                  className="text-ink-5 hover:text-red-400 transition-colors shrink-0 disabled:opacity-50"
                >
                  <svg
          ...
          {addingMember ? (
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-xs text-ink-4 hover:text-ink transition-colors mt-1.5 pt-0.5"
            >
              <svg
                className="w-3.5 h-3.5"
                fill="none"
        ...
        {/* Active SMR Assignments & Locations */}
        <div className="border-t border-line pt-3 space-y-2">
          <div className="flex items-center justify-between">
            <p className="text-[10px] font-medium uppercase tracking-wider text-ink-4">
              Активни обекти и СМР ({activeSmrs.length})
            </p>
          </div>

          {activeSmrs.length === 0 ? (
            <p className="text-xs text-ink-5 py-1">
              Бригадата няма активни СМР задачи.
            </p>
          ) : (
            <div className="space-y-2">
              {activeSmrs.map((s) => {
                const progressPercent = Math.min(
                  100,
                  Math.round((s.done / (s.quantity || 1)) * 100),
                );
                ...
                return (
                  <Link
                    ...
                    className="block p-2.5 bg-well rounded-xl border border-line/60 hover:border-line-2 hover:bg-lift transition-all group"
                  >
                    {/* Location Breadcrumb Path */}
                    <div className="flex items-center gap-1 text-[11px] font-medium text-ink-3 mb-1 min-w-0">
                    ...
                    <div className="flex items-center justify-between gap-2 mb-2">
                      <p className="text-xs font-medium text-ink truncate group-hover:text-ink transition-colors">
                        {s.name}
                      </p>
                      <span className="text-[10px] font-semibold text-ink-3 tabular-nums shrink-0">
                        {s.done} / {s.quantity} {s.unit}
                      </span>
                    </div>

                    {/* Visual Progress Bar */}
                    <div className="space-y-1">
                      <div className="flex justify-between items-center text-[10px] text-ink-4">
                        <span>Напредък</span>
...
export default function BrigadiClient({
  initialBrigades,
}: {
  initialBrigades: BrigadeWithAll[];
}) {
        ...
        {initialBrigades.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-xs text-ink-5 mt-1">
              Създайте първата бригада, за да я назначите на СМР.
            </p>
          </div>
        ) : (
